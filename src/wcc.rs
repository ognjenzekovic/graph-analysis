...
mod tests {
    use super::*;
    use crate::wcc::wcc_sequential; // Za poređenje sa sekvencijalnom

    #[test]
    fn test_union_find_basic() {
        let mut uf = UnionFind::new(5);

        // Inicijalno: svaki čvor je u svojoj komponenti
        ...
        // Spoji 0 i 1
        uf.union(0, 1);
        assert_eq!(uf.find(0), uf.find(1)); // Sada su u istoj komponenti

        // Spoji 1 i 2 (indirektno spaja i 0 i 2)
        uf.union(1, 2);
        assert_eq!(uf.find(0), uf.find(2)); // 0 i 2 su sad u istoj komponenti

        // 3 i 4 su još uvek odvojeni
    ...
    fn test_wcc_simple() {
        ...
        // 0, 1, 2 trebaju biti u istoj komponenti
        assert_eq!(result[0], result[1]);
        assert_eq!(result[1], result[2]);

        // 3, 4 trebaju biti u istoj komponenti
        assert_eq!(result[3], result[4]);

        // Ali različitoj od {0,1,2}
        assert_ne!(result[0], result[3]);
    }

    #[test]
    fn test_wcc_all_connected() {
        // Graf: 0→1→2→3 (jedna komponenta)
        let graph = Graph {
            num_nodes: 4,
        ...
        // Svi čvorovi u istoj komponenti
        assert_eq!(result[0], result[1]);
        assert_eq!(result[1], result[2]);
    ...
    fn test_wcc_all_disconnected() {
        // Graf bez ivica (svaki čvor je komponenta)
        let graph = Graph {
            num_nodes: 4,
        ...
        // Svaki čvor je u svojoj komponenti
    ...
    fn test_wcc_parallel_vs_sequential() {
        ...
        // Rezultati mogu imati različite ID-jeve,
        // ali odnosi između čvorova moraju biti isti

        // 0,1,2 u istoj komponenti
        assert_eq!(
    ...
    #[test]
    fn test_wcc_parallel_large_graph() {
        // Test sa malo većim grafom
        use crate::graph_generator::generate_random;

        // Generiši random graf
        ...
        // Broj komponenti mora biti isti
